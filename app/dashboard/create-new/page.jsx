...
const FILEURL = 'https://firebasestorage.googleapis.com/v0/b/projetos-536cd.firebasestorage.app/o/arquivos-shorts-de-ai%2F11d8aa7a-ade2-4584-8949-ed0e6cd7e1fc.mp3?alt=media&token=1af6ed91-e03d-4c09-be1d-ada635334cac'

const VideoSCRIPT = [
  {
    imagePrompt: "ator terry crews segurando o perfume árabe celeste",
    contentText: "ator terry crews segurando o perfume árabe celeste",
  },
  {
    imagePrompt:
      "A close-up of a determined-looking Australian farmer, wearing a wide-brimmed hat, looking frustrated at damaged crops, realistic, photorealistic.",
    contentText:
      "These large, flightless birds, driven by drought, invaded farmlands, devouring crops and destroying fences, threatening the livelihoods of the struggling population.",
  },
];
...
function CreateNew() {
  ...
  const GetVideoScript = async () => {
    setLoading(true);
    const prompt =
      "Escreva um script que gere " +
      formData.duration +
      " um vídeo do tópico : " +
      formData.topic +
      " junto com o prompt de imagem de IA no estilo " +
      formData.imageStyle +
      " para cada cena e me dê o resultado em JSON com o imagePrompt and ContentText as field, sem textos simples";
    console.log(prompt);
  ...
  /*Usado para gerar as imagens de IA*/
  const GenerateImage = async () => {
    setLoading(true);

    try {
      const reqs = VideoSCRIPT.map((item) =>
        axios.post("/api/image/generate-image", {
          prompt: item.imagePrompt,
        })
      );

      const results = await Promise.all(reqs);
      const images = results.map((r) => r.data.result);
      console.log(images);
      setImageList(images);
    } catch (e) {
      console.error(e);
